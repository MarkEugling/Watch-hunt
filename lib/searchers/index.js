...
import { search as searchDealers, SOURCE as DEALERS } from './dealers.js'
import { computeTrust, normalizeSeller } from '../trust.js'
...
const ALL_SEARCHERS = [
  ...
  { name: DEALERS, fn: searchDealers, type: 'dealer' },
]

// eBay only participates when Browse API credentials are configured —
// without them it has no real data to offer, so it's omitted entirely
const SEARCHERS = ALL_SEARCHERS.filter(s =>
  s.name !== EBAY || (process.env.EBAY_CLIENT_ID && process.env.EBAY_CLIENT_SECRET)
)
 ...
 */
export async function runAllSearches({ query, brand, model, reference, maxPrice }) {
  ...
  const settled = await Promise.allSettled(
    SEARCHERS.map(async ({ name, fn, type }) => {
      ...
      try {
        const results = await fn({ query, brand, model, reference, maxPrice })
  ...
  for (const outcome of settled) {
    ...
    if (normRef) {
      filtered = results.filter(r => {
        // Always keep browse/search links — they're navigational, not individual listings
        if (r.isLink) return true
        // Keep the result only if the title contains the reference number
        return norm(r.title).includes(normRef)
      })
    }

    sourceStatus[name] = {
      count: filtered.filter(r => !r.isLink).length,
      ms,
      error,
      type,
    }
    ...
    for (const r of filtered) {
      const enriched = { ...r, sourceType: type, seller: normalizeSeller(r.seller) }
      enriched.trust = computeTrust(enriched)
