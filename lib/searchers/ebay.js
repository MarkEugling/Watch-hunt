...
function searchUrl(keywords, { maxPrice, sold = false } = {}) {
...
export async function search({ query, brand, model, reference, maxPrice }) {
  ...
  try {
    ...
    results.push({
      source: SOURCE,
      title: `Recently sold: "${baseQuery}"`,
      price: null,
      priceDisplay: `≤ $${maxPrice.toLocaleString()}`,
      saleDate: null,
      saleName: 'Search link · Sold prices',
      url: searchUrl(baseQuery, { maxPrice, sold: true }),
      imageUrl: null,
      currency: 'USD',
      isLink: true,
    })
