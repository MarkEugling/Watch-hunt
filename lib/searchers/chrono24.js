...
export async function search({ query, brand, model, reference, maxPrice }) {
  ...
  const searchPageUrl = `https://www.chrono24.com/search/index.htm?query=${encodedQuery}&dosearch=true&priceTo=${maxPrice}&currencyId=USD&sortorder=1`
  ...
  // Always include the live search link — guaranteed-correct destination
  results.push({
    source: SOURCE,
